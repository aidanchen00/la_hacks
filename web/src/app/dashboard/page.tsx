...
function DashboardContent() {
  ...
  const redditDefaults = useMemo(() => {
    ...
    const titleBase = titleSummary || `My Prana intake — ${pathLabel}`;
    const title = `[Prana intake] ${titleBase}`.slice(0, 300);

    const bodyParts: string[] = [];
    bodyParts.push(`**Urgency:** ${rd.urgency} · **Recommended path:** ${pathLabel}\n`);
    if (tSummary ?? rd.summary) {
      bodyParts.push(`### Summary\n${tSummary ?? rd.summary}\n`);
    }
    if (rd.next_actions?.length) {
      bodyParts.push("### Next actions");
    ...
    bodyParts.push("---");
    bodyParts.push("_Generated by Prana, an AI wellness navigation tool. Not a medical diagnosis. For emergencies, call 911._");
    return { title, body: bodyParts.join("\n") };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rd, tSummary, tNextActions]);
