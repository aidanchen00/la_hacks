...
Sends a ChatMessage to the CareFlow Agentverse agent and prints the RoutingDecision response.
...
# The CareFlow agent's Agentverse address (set via CLI or env)
CAREFLOW_ADDRESS = os.getenv("CAREFLOW_AGENT_ADDRESS", "")


def main():
    parser = argparse.ArgumentParser(description="Probe CareFlow agent via Chat Protocol")
    parser.add_argument("--to", default=CAREFLOW_ADDRESS, help="CareFlow agent address")
    parser.add_argument("--text", required=True, help="Health/wellness concern to send")
    args = parser.parse_args()

    if not args.to:
        print("❌ No agent address. Set CAREFLOW_AGENT_ADDRESS env or use --to <address>")
        sys.exit(1)

    probe = Agent(name="omegaclaw-probe", seed="omegaclaw-probe-seed-careflow-2026", port=8200)
    chat_proto = Protocol(spec=chat_protocol_spec)
    received_reply = asyncio.Event()

    @chat_proto.on_message(ChatMessage)
    async def on_reply(ctx: Context, sender: str, msg: ChatMessage):
        print(f"\n✅ CareFlow reply from {sender[:30]}:")
    ...
    async def send_message(ctx: Context):
        print(f"\n🩺 Sending to CareFlow ({args.to[:30]}…):")
        print(f"   \"{args.text}\"\n")
        await ctx.send(
            args.to,
            ChatMessage(
    ...
    async def timeout(ctx: Context):
        if received_reply.is_set():
            return
        print("⏰ Timeout — no reply after 30s. Check if CareFlow agent is running.")
        await ctx.stop()
